...
class CoopEnv(gym.Env):
    ...
    def __init__(self, n, task_num, percentage):
        ...
        self.termination_percentage = percentage
        self.done = False
        self.entropy = 0 # measure of the number of coalition changes 
        # -------------------- #
    ...
    def step(self, actions): # expect 'actions' to be a dictionary list of actions for each player
        ...
        self.entropy = 0 # reset entropy for this step
        ...
        for agent in range(self.n): # action will be index of task to join
            ...
            if new_coalition != current_coalition:
                
                self.entropy += 1
            
            #print(f'current CS is: {self.CS}')
            #print(f'target coalition is {self.CS[new_coalition]}')
            #print(f'current coalition is {self.CS[current_coalition]}')
            
            #print('Performing transition...')
            ...
            #print(f'new CS is: {self.CS}')
            #print(f'new target coalition is {self.CS[new_coalition]}')
            #print(f'new current coalition is {self.CS[current_coalition]}')
            #print('\n')
        ...
        # ------------------------------------------- #
        if self.entropy <= self.termination_percentage*self.n: # if number of coalition changes < specified proportion of players
            
            self.done=True
            
        info = [self.CS, self.entropy]
    ...
    def reset(self, n, task_num, percentage):
